...
const P=[
 {id:'dro',d:1,base:'dur',cat:'transport',lat:37.1515,lng:-107.7538,nm:'Durango–La Plata County Airport (DRO)',q:'Durango-La Plata County Airport',pin:'терминал, а не середина лётного поля',tag:['прилёт','t-easy'],
 ...
 {id:'trn',d:2,base:'dur',cat:'transport',lat:37.2740,lng:-107.8800,nm:'Durango & Silverton Narrow Gauge Railroad',q:'Durango & Silverton Narrow Gauge Railroad',pin:'депо на Main Avenue, откуда отходит поезд, а не станционные пути',tag:['ради этого едут','t-must'],star:1,when:'fixed',
 ...
 {id:'mvp',d:3,base:'dur',cat:'town',lat:37.3086,lng:-108.4187,nm:'Mesa Verde National Park',pin:'въезд и визит-центр у US-160, а не середина парка',q:'Mesa Verde National Park',tag:['ЮНЕСКО','t-must'],star:1,
 ...
 {id:'vlc',d:3,base:'dur',cat:'nature',lat:37.3800,lng:-107.5600,opt:'lake',nm:'Vallecito Lake',q:'Vallecito Lake',pin:'берег озера в 40 мин от Дуранго; ярлык справочника стоит в 35 км выше, в верховьях ручья',tag:['спокойный вариант','t-easy'],
  why:'Если скальные города не хочется — большое тихое горное озеро в 40 мин. Сап, пикник.'},
 {id:'hav',d:4,base:'our',cat:'nature',lat:37.534600,lng:-107.810300,nm:'Haviland Lake',q:'Haviland Lake Colorado',
  why:'Первая остановка за Дуранго: тихое лесное озеро у трассы. Кофе перед подъёмом.'},
 {id:'cbp',d:4,base:'our',cat:'nature',lat:37.700600,lng:-107.777000,nm:'Coal Bank Pass',q:'Coal Bank Pass',
 ...
 {id:'bcg',d:6,base:'asp',cat:'nature',lat:38.5550,lng:-107.6866,nm:'Black Canyon of the Gunnison NP',pin:'южный борт: смотровые и дорога от Монтроза, а не середина каньона',q:'Black Canyon of the Gunnison National Park',tag:['нацпарк №1','t-must'],star:1,
 ...
 {id:'red',d:6,base:'asp',cat:'town',lat:39.168889,lng:-107.241667,nm:'Redstone + Redstone Castle',q:'Redstone Castle',
  why:'Посёлок вдоль красной реки Crystal с замком угольного магната 1902 г. Кофе перед броском в Аспен.'},
 {id:'mrb',d:7,base:'asp',cat:'nature',lat:39.0975,lng:-106.9403,nm:'Maroon Bells + Maroon Lake',q:'Maroon Bells',pin:'парковка у Maroon Lake, куда приходит автобус, а не сами вершины',tag:['открытка штата','t-must'],star:1,
 ...
 {id:'dil',d:9,base:'den',cat:'nature',lat:39.588700,lng:-106.044100,nm:'Sapphire Point / Dillon Reservoir',q:'Sapphire Point Overlook',
...
const HERO={
  sub:'Поперёк Скалистых гор: паровоз на Силвертон, скальные города Меса-Верде, Million Dollar Highway без отбойников, Чёрный каньон и перевал Independence на 3 687 метрах.',
  ...
  place:'mrb',
  parks:'2',parksCap:'национальных парка'
};

/* ── СВОДКА ПЕРЕЕЗДОВ ──
   Раньше эта таблица стояла готовым текстом в index.html и висела под любым
   маршрутом — под Парижем и Токио тоже. Теперь она там, где ей место: в данных
   своей поездки. Цифры прежние, ничего не пересчитано. */
const DRIVE={
  rows:[
    {d:'1',seg:'Прилёт DRO → Дуранго',km:'24 км',t:'25 мин',stops:'вечер'},
    {d:'2',seg:'Поезд в Силвертон (без машины)',km:'—',t:'—',stops:'6–9 ч'},
    {d:'3',seg:'Дуранго ↔ Меса-Верде',km:'170 км',t:'2 ч',stops:'весь день'},
    {d:'4',seg:'Дуранго → Урей (Million Dollar Hwy)',km:'120 км',t:'2 ч',stops:'5–6 ч'},
    {d:'5',seg:'Урей ↔ Теллурайд',km:'150 км',t:'2,5 ч',stops:'весь день'},
    {d:'6',seg:'Урей → Аспен (Чёрный каньон)',km:'330 км',t:'5 ч',stops:'весь день'},
    {d:'7',seg:'Аспен ↔ Maroon Bells',km:'30 км',t:'30 мин',stops:'весь день'},
    {d:'8',seg:'Аспен ↔ долины Castle Creek',km:'60 км',t:'1 ч',stops:'весь день'},
    {d:'9',seg:'Аспен → Денвер (Independence Pass)',km:'260 км',t:'4 ч',stops:'6–7 ч'},
    {d:'10',seg:'Денвер → аэропорт DEN, вылет',km:'—',t:'30 мин',stops:'утро'}
  ],
  total:{seg:'Дуранго → Денвер',km:'~1 150 км',t:'~18 ч',stops:'10 дней'}
};

/* ── ЧТО ЗАНЯТЬ ЗАРАНЕЕ И ПРАКТИКА ──
   Тоже переехало сюда из index.html по той же причине. Первый блок раскрыт. */
const TIPS=[
 {t:'Что бронировать заранее (по срочности)',open:true,li:[
  '<b>Меса-Верде (Cliff Palace) — критично.</b> Билеты открываются ровно за 14 дней в 08:00 по местному времени (MDT). День в парке 9 августа → релиз <b>26 июля</b>. Разбирают за минуты, только <a href="https://www.recreation.gov/ticket/facility/233362" target="_blank" rel="noopener">recreation.gov</a>. В сам парк пускают и без билета.',
  '<b>Maroon Bells — критично.</b> Август пик. Парковка $10 (только до 08:00 или после 17:00) либо шаттл $16. <a href="https://www.visitmaroonbells.com/" target="_blank" rel="noopener">visitmaroonbells.com</a>.',
  '<b>Поезд Durango &amp; Silverton.</b> Открытые вагоны на август берут заранее. <a href="https://durangotrain.com/" target="_blank" rel="noopener">durangotrain.com</a>.',
  '<b>Машина Дуранго → Денвер (one-way).</b> Сбор за возврат в другом городе ~$100-150. SUV разбирают первыми.',
  '<b>Джип-тур Yankee Boy Basin</b> — опционально, из Урея, полдня.']},
 {t:'Погода и грозы в августе',li:[
  'Сезон муссонов: почти каждый день после 13:00–14:00 короткая гроза в горах.',
  'Правило: всё высокогорное — до полудня; города, музеи, источники — после.',
  'Долины +25…30°, перевалы +10…15°, ночью до +5°. Нужны слои и ветровка.',
  'Солнце на 3 000 м жжёт втрое — крем 50+.']},
 {t:'Машина и дороги',li:[
  'Обычной легковой хватает на 95%. SUV нужен только для Yankee Boy, Last Dollar Road, верхней Bridal Veil.',
  'Independence Pass: нельзя ТС длиннее 10,7 м.',
  'Million Dollar Highway — без отбойников. Не ночью и не в дождь.',
  'Между Уреем и Силвертоном заправок нет — заправляйся заранее.',
  'Связь пропадает на перевалах — скачай офлайн-карты Google Maps на юго-запад Колорадо.',
  'Статус дорог: <a href="https://www.cotrip.org/" target="_blank" rel="noopener">cotrip.org</a>.']},
 {t:'Входы в парки',li:[
  'Black Canyon $30/машина, Mesa Verde $30/машина — только карта, наличные не берут.',
  'Годовой <a href="https://www.nps.gov/planyourvisit/passes.htm" target="_blank" rel="noopener">America the Beautiful</a> $80 окупается от трёх парков — тут два.']},
 {t:'Если появится лишний день',li:[
  '<b>Great Sand Dunes</b> — раз вылетаешь из Денвера, можно с дня 9 южным путём US-160 через дюны (+1 день).',
  '<b>Rocky Mountain NP</b> — 2 ч севернее Денвера; заменить ночь у аэропорта на Estes Park.',
  '<b>Crested Butte</b> — «столица полевых цветов», крюк с дня 6.']}
];
...
const META={
 dro:{min:40,price:'—',dur:'приезд',pid:'ChIJ-fBCJJAZPIcRzt7f27O1Rek'},
 ...
 vlc:{min:120,price:'бесплатно',route:'~40 мин от Дуранго',pid:'ChIJG-GmePnsPocRKZaH68T5Mag'},
 pag:{min:180,price:'от $67/чел',route:'~1 ч на восток',pid:'ChIJSeBtWGDbPYcR4zuLh1QqcV4'},
 chr:{min:150,price:'$12 +$20/маш',best:'днём',route:'~1 ч от Дуранго',pid:'ChIJlXopPMYyPIcRclB12F4YWpk'},
 hav:{min:20,price:'бесплатно',route:'по пути',pid:'ChIJj0ncStW7PocR_hMwjVW20Oo'},
 cbp:{min:15,price:'бесплатно',route:'по пути',pid:'ChIJRUrCwaXGPocRG9OqPAujTGs'},
 mol:{min:25,price:'бесплатно',best:'днём',route:'по пути',pid:'ChIJAQAAJz_CPocRUqTj_j4oI_E'},
 sil:{min:90,price:'обед',best:'обед',route:'по пути',pid:'ChIJTT8-1PjnPocRP8t9tS4432g'},
 mdh:{min:60,price:'бесплатно',best:'днём, не в грозу',route:'сама дорога',pid:'ChIJV-kQXEfePocRx1L0tuXVHv4'},
 rmp:{min:30,price:'бесплатно',route:'по пути',pid:'ChIJd_XMGWHePocRKDqBNsesnuI'},
 bcf:{min:10,price:'бесплатно',route:'по пути',pid:'ChIJJ9XSHY8fP4cRbh5jW7cQg7w'},
 ohs:{min:90,price:'$26/чел',best:'вечер',route:'в Урее',pid:'ChIJbx3UWd4hP4cR8f8tNFcduuo'},
 box:{min:45,price:'$8/чел',best:'днём',route:'в Урее',pid:'ChIJTRwrydYhP4cRHFzanEA71p4'},
 per:{min:90,price:'бесплатно',best:'днём',route:'в Урее',pid:'ChIJk7u1W2IfP4cRmeTpo82g7OI'},
 ykb:{min:180,price:'~$125/чел (джип)',best:'до полудня',route:'джип-тур из Урея',pid:'ChIJLw5vQ9gmP4cR5WtGI53gkB8'},
 tel:{min:150,price:'гондола бесплатно',best:'днём / закат',route:'~1 ч 15 от Урея',pid:'ChIJTzJyHmPYPocRc1OGJ0U10Bc'},
 ...
 red:{min:30,price:'бесплатно',route:'по пути',pid:'ChIJ-y9PfdRZQIcR8BAGzI3XF3s'},
 mrb:{min:120,price:'шаттл $16 / парк. $10',best:'на рассвете',route:'~30 мин от Аспена',pid:'ChIJF6-9fetBQIcRxzv76neQ_yo'},
 mls:{min:60,price:'бесплатно',best:'утро',route:'у озера',pid:'ChIJnX3HPF9AQIcRHFSRpMeb9V8'},
 crl:{min:120,price:'бесплатно',best:'утро',route:'от Maroon Lake',pid:'ChIJlRQeCa5BQIcRVDGovSoPbN8'},
 gon:{min:90,price:'от $40/чел',best:'после обеда',route:'центр Аспена',pid:'ChIJMRLNe2I5QIcR1qMXPWcFqVA'},
 asp:{min:45,price:'бесплатно',best:'вечер',route:'центр Аспена',pid:'ChIJQbziSNY7QIcRHp0iwpSWqpA'},
 gro:{min:60,price:'бесплатно',best:'до 10:00',route:'~20 мин от Аспена',pid:'ChIJB6kyXYw7QIcRMJcRtMjxbAE'},
 dpb:{min:30,price:'бесплатно',best:'днём',route:'у Grottos',pid:'ChIJjQxBh8YiQIcRkunT61fXwC4'},
 ash:{min:60,price:'$5/чел',best:'днём',route:'~25 мин от Аспена',pid:'ChIJV-vZFWkWQIcRA2Xv7CUzAmg'},
 cth:{min:270,price:'бесплатно',dur:'4–5 ч (хайк)',best:'выход до 8:00',route:'за Ashcroft',pid:'ChIJkY22knwWQIcR6d5K0te9Y_M'},
 rgt:{min:150,price:'прокат вело',best:'днём',route:'из Аспена',pid:'Eh5SaW8gR3JhbmRlIFRybCwgQXNwZW4sIENPLCBVU0EiLiosChQKEgmf3RGYNE1AhxH8uadBGO1sHRIUChIJfTxB93w5QIcRcvYseNxCK8E'},
 igh:{min:20,price:'бесплатно',route:'по пути',pid:'ChIJ1b951T0iQIcRF8vBYV5FMjQ'},
 ind:{min:30,price:'бесплатно',best:'до полудня',route:'по пути (перевал)',pid:'ChIJcS3UI4wfQIcRf0ORHl22MRQ'},
 twn:{min:30,price:'бесплатно',best:'утро',route:'по пути',pid:'ChIJcYquhAujaocRjmvL2C84pTE'},
 dil:{min:30,price:'бесплатно',best:'днём',route:'по пути',pid:'ChIJmxWGNFxZaocRCOFIsTnaGs8'},
 lov:{min:25,price:'бесплатно',best:'днём',route:'крюк ~25 мин',pid:'ChIJLaVZ3oZRaocRB136_KKmnSc'},
 geo:{min:45,price:'бесплатно (город)',best:'днём',route:'по пути',pid:'ChIJx08VvhZTaocR7CI8VHxQvUs'},
 den:{min:180,price:'—',dur:'вылет',pid:'ChIJ_0T_mCp_bIcRapy1NbQ7WEk'}
};
...
const ROADS={
 ...
