...
function build(){
  ...
  /* ⚠️ Здесь стояло «не забудьте добавить папки в .gitignore-исключения» — то
     есть защита, которая держится на памяти. С 08.08.2026 (шаг 3 плана) это
     проверяет `node check-ship.js`, и без него выкладка не идёт вовсе. */
  console.log('Проверить, всё ли поедет на сайт: node check-ship.js');
}
