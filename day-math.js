...
(function (root, factory) {
  ...
  function dayCost(items, opt) {
    ...
    for (var i = 0; i < list.length; i++) {
      ...
      for (var m = 0; m < meals.length; m++) {
        var ml = meals[m];
        var early = (opt.early == null ? EARLY : opt.early);
        if (eaten[ml.key] || clock < ml.from - early || clock > ml.to) continue;
        ...
        var jumps = after > ml.to;     /* уйдём дальше — окно проскочим целиком */
        /* окно ещё не открылось: ждём, если успеваем вернуться в него. А если
           впереди Лувр на три с половиной часа — едим сейчас, до него. Иначе
           обед не случался вовсе и день выходил на час дешевле, чем он есть */
        if (clock < ml.from && !jumps) continue;
        var mustNow = jumps || (dur >= longPlace && after > mid + ml.min);
  ...
  var CASES = [
    ...
    {
      nm: 'Лувр: ушёл в 11:30 на 3,5 часа — кормим ДО окна, а не «никогда»',
      items: [{ move: 20, min: 130 }, { move: 20, min: 210 }],
      total: 380 + 60, food: 60, tail: 1, mealBefore: 1
    },
    {
      nm: 'окно ещё не открылось, но мы успеем вернуться — не торопимся',
      items: [{ move: 20, min: 100 }, { move: 20, min: 60 }, { move: 20, min: 120 }],
      total: 340 + 60, food: 60, tail: 1, mealBefore: 2
    },
    {
