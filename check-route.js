...
function checkRoute(file) {
  ...
  /* ── НОЧЬ В ПУТИ БЫВАЕТ ТОЛЬКО В ДОЛГОЙ ДОРОГЕ ──
     Её вопрос по пробному маршруту: «поезд идёт два часа — где мы тогда будем
     ночевать, приедем в час ночи?». Верно: ночь в пути — это когда человек
     ДЕЙСТВИТЕЛЬНО спит в дороге (ночной поезд, паром, дальний рейс), а не когда
     он приезжает ночью и остаётся на улице. Меньше шести часов — это не ночь. */
  if (S.TRANSFER) {
    Object.keys(S.TRANSFER).forEach(bid => {
      const t = S.TRANSFER[bid];
      if (!t || !t.night) return;
      const mins = parseMin(t.clean || '');
      const nm = ((BASES || []).find(b => b.id === bid) || {}).name || bid;
      if (!mins) { warn('переезд в «' + nm + '»: объявлена ночь в пути, но не написано, сколько ехать (clean) — проверить нечем'); return; }
      if (mins < 360) bad('переезд в «' + nm + '»: объявлена ночь в пути («' + t.night + '»), а ехать '
        + Math.floor(mins / 60) + ' ч ' + (mins % 60) + ' мин. Так человек приедет среди ночи, и спать ему негде: '
        + 'либо это не ночной переезд (уберите night), либо в городе нужна обычная ночь');
    });
  }
