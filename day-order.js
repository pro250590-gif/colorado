...
function analyze(S) {
  ...
  return days.map(d => {
    ...
    /* Анкер — точка с ролью «дорога»: вокзал, причал, аэропорт, всё, к чему
       добираются расписанием, и всё, что стоит по часам. Плюс любой
       написанный руками hop: автор объяснил, как попадают в эту точку ИЗ
       ПРЕДЫДУЩЕЙ («10 минут по Cliff Palace Loop», «катер 45 минут»), значит
       пара задана. Один язык ролей на все инструменты — правило 17. */
    const anchor = (p, i) => i > 0 && (!!p.hop || p.when === 'fixed' || p.cat === 'transport');
    const runs = [];
